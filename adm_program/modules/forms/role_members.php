...
$getMode        = 'csv';
$getListId      = 170; //Kontaktdaten Startbuch
            ...
            || $gProfileFields->getPropertyById($usf_id, 'usf_hidden') == 0)
            {
              if($column_number == 1)
              {
                ...
                ||     $column->getValue('lsc_special_field') == 'mem_end') 
                {
                    ...
                    {
//print_r($row[$sql_column_number]);echo" ::DEBUG:row[sql_column_number]<br>";
                        // date must be formated
                        $date = new DateTimeExtended($row[$sql_column_number], 'Y-m-d', 'date');                      
                        //$date = new DateTime($row[$sql_column_number]);                      
                        $content = $date->format($gPreferences['system_date']);

//print_r($date);echo" ::DEBUG:date<br>";
//print_r($content);echo" ::DEBUG:content<br>";
