...
if($getJob=='save')
{
    ...
    if($gPreferences['profile_photo_storage'] == 1)
    {
        ...
        if(file_exists(SERVER_PATH. '/adm_my_files/user_profile_photos/'.$getUserId.'a_new.jpg'))
        {
            if(file_exists(SERVER_PATH. '/adm_my_files/user_profile_photos/'.$getUserId.'a.jpg'))
            {
                unlink(SERVER_PATH. '/adm_my_files/user_profile_photos/'.$getUserId.'a.jpg');
            }
            
            rename(SERVER_PATH. '/adm_my_files/user_profile_photos/'.$getUserId.'a_new.jpg', SERVER_PATH. '/adm_my_files/user_profile_photos/'.$getUserId.'a.jpg');
        }
    }
    else
    {
...
elseif($getJob=='dont_save')
{
    ...
    if($gPreferences['profile_photo_storage'] == 1)
    {
        if(file_exists(SERVER_PATH. '/adm_my_files/user_profile_photos/'.$getUserId.'a_new.jpg'))
        {
            unlink(SERVER_PATH. '/adm_my_files/user_profile_photos/'.$getUserId.'a_new.jpg');
        }
    }
...
if(strlen($getJob) == 0)
{
    ...
    echo '
    <form method="post" action="'.$g_root_path.'/adm_program/modules/profile/profile_photo_edita.php?job=upload&amp;usr_id='.$getUserId.'" enctype="multipart/form-data">
    <div class="formLayout" id="profile_photo_upload_form">
        <div class="formHead">'.$headline.'</div>
        <div class="formBody">
            <p>'.$gL10n->get('PRO_CURRENT_PICTURE').' Dame:</p>
            <img class="imageFrame" src="profile_photo_showa.php?usr_id='.$getUserId.'" alt="'.$gL10n->get('PRO_CURRENT_PICTURE').'" />
...
elseif($getJob == 'upload')
{
    ...
    $user_image->copyToFile(null, SERVER_PATH. '/adm_my_files/user_profile_photos/'.$getUserId.'a_unscaled.jpg');
    ...
    $user_image->copyToFile(null, SERVER_PATH. '/photos/'.$getUserId.'a.jpg');
    
    //Ordnerspeicherung
    if($gPreferences['profile_photo_storage'] == 1)
    {
        $user_image->copyToFile(null, SERVER_PATH. '/adm_my_files/user_profile_photos/'.$getUserId.'a_new.jpg');
    }
    //Datenbankspeicherung
    else
    {
        ...
        <div class="formBody">
            <table style="border: none; width: 100%; padding: 5px;">
                <tr style="text-align: center;">
                    <td>'.$gL10n->get('PRO_CURRENT_PICTURE').' Dame:</td>
                    <td>'.$gL10n->get('PRO_NEW_PICTURE').' Dame:</td>
                </tr>
                <tr style="text-align: center;">
                    <td><img class="imageFrame" src="profile_photo_showa.php?usr_id='.$getUserId.'" alt="'.$gL10n->get('PRO_CURRENT_PICTURE').'" /></td>
                    <td><img class="imageFrame" src="profile_photo_showa.php?usr_id='.$getUserId.'&new_photo=1" alt="'.$gL10n->get('PRO_NEW_PICTURE').'" /></td>
                </tr>
            </table>
            ...
            <div class="formSubmit">
                <button id="btnCancel" type="button" onclick="self.location.href=\''.$g_root_path.'/adm_program/modules/profile/profile_photo_edita.php?job=dont_save&amp;usr_id='.$getUserId.'\'">
                    <img src="'.THEME_PATH.'/icons/error.png" alt="'.$gL10n->get('SYS_ABORT').'" />
                    &nbsp;'.$gL10n->get('SYS_ABORT').'
                </button>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
                <button id="btnUpdate" type="button" onclick="self.location.href=\''.$g_root_path.'/adm_program/modules/profile/profile_photo_edita.php?job=save&amp;usr_id='.$getUserId.'\'">
