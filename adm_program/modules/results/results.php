...
$html_rgbnss .= '
  <p><a name="Ergebnissliste"></a><hr /><br><b>Ergebnissliste & Wertungen f&uuml;r: '.$_POST["mitglied_nummer"].'</b></p> 
  <table border="0" width="720px" cellspacing=2 cellpadding=3 style="font-size: 11px;">
    <tr bgcolor=33cc00>
      <td align=center><b>Datum</b></td>
      <td><b>Turnier</b></td>
      <td align=center><b>RglPkt.</b></td>
      <td align=center><b>AufPkt.</b></td>
      <td align=center><b>&sum; FT</b></td>
      <td align=center><b>&sum; Tanz</b></td>
      <td align=center><b>&sum; Akro</b></td>
      <td align=center><b>Ergebnis</b></td>
      <td align=center><b>Platzierung</b></td>  
      <td align=center><b>Wertungen</b></td>  
    <tr>';
...
while($platzierungen = mysqli_fetch_array($platz)){
  ...
  $html_rgbnss  .= '
        ...
        <td align=center><b>'.$platzierungen["platz"].'. Platz<b></td>
        <td align=center>           
...
if ($html_rgbnss == ''){
  $html_rgbnss = '<li><dl><dt>'.date("d.m.Y",time()).'</dt><dd><b><i>Es liegen noch keine Turnierergebnisse vor.</i></b></dd></dl></li>'; 
}   
...
if(substr($paarinfo["startklasse"],0,2) == "RR"){  
  $tanzhead_ar = array("Herr Grundtechnik","Herr Haltungs&Drehtechnik","Dame Grundtechnik","Dame Haltungs&Drehtechnik","Choreografie","Tanzfiguren","TÃ¤nzerische Darbietung");
  ...
  for ($runden_cnt = 1; $runden_cnt <= count($runde); $runden_cnt++){
      ...
      $html_vrbw .= '
         ...
         $html_vrbw .= '
            <tr style="background-color: #F5A9A9;">
               <td align=right><b>Abz&uuml;ge Tanz&nbsp;</b></td>
               <td align=center colspan='.(2*$twrnr).'><b></b></td>
             </tr>';
         $html_vrbw .= '
            <tr style="background-color: #BCF5A9;">
               <td align=center colspan='.(1+2*$twrnr).'><b>Ergebnis = '.number_format($tanzpkte_sum,2).' Pkt.'.$compare_html.'</b></td>
             </tr>';
    ...
    if($runde_name == "Endrunde Fußtechnik"){
      ...
      $html_ft .= '
         ...
         if($tanzpkte_sum <= $t_abzg_pkte[0]){
           ...
           $html_ft .= '<td align=center colspan=9><b>Ergebnis = '.number_format($tanzpkte_sum-$t_abzg_pkte[0],2).' Pkt.'.$compare_html.'</b></td>';                            
         }
         $html_ft .= '
            </tr>
          </table>';              
    ...
    || $runde_name == "1. Zwischenrunde" || $runde_name == "2. Zwischenrunde" || $runde_name == "3. Zwischenrunde")){      
      ...
      $html_ak    .= '
         ...
         if($startklasse_mit_akro){
           $html_ak .= '<td align=left colspan='.(13+$colspan_add).'><b>Abz&uuml;ge Akrobatik&nbsp;</b></td>';
         ...
         if($startklasse_mit_akro){                 
           ...
           $html_ak .= '<td align=center colspan='.($colspan_value + 1).'><b>Ergebnis = ';         
               ...
               if(($tanzpkte_sum-$t_abzg_pkte[0]+$akropkte_sum-(array_sum($a_abzg_pkte)/($awrnr))) > 0){
                 ...
                 $html_ak .= number_format($tanzpkte_sum-$t_abzg_pkte[0]+$akropkte_sum-(array_sum($a_abzg_pkte)/($awrnr)),2).' Pkt.'.$compare_html.'</b></td>';                 
               } else {
                 $erg_pkt_su = 0;
...
$html_wrtg    = '
  <p><hr /><br><b>Wertungen f&uuml;r: '.$turniername.'</b></p>';
...
echo'<h1 class="moduleHeadline">Wertungsergebnisse</h1>';
echo'<div class="formLayout" id="profile_form">';
echo'  <div class="formHead">Startbuchinformation</div>';
echo'    <form method="POST" action="' . $_SERVER["PHP_SELF"] . '">';
echo'    <div class="formBody" align="left">';
echo'      <font face="Verdana" size="3" color="#000080">
             <p align="left">Bitte folgenden Daten für die Wertungsfreigabe eingeben:<br></p>
             <table>
               <tr align="left">
                 <td>Startbuch-/Startkartennummer:</td>
               ...
               <tr align="left">
                 <td>&nbsp;</td>
                 <td><input type="submit" name="senden" value="Wertungen anzeigen!"/></td>
               </tr>            
             </table>    
           </font>';                                
...
{
  $alert_stnr_fehlt = '<h3>Bitte eine Startbuchnummer eingeben!</h3>';
} else {
  $nicht_vorhanden = 0;
  // Startbuch finden
...
if($abbruch){                 
  ...
  if($startklasse == "F_RR_J"  || 
     ...
     $startklasse == "F_RR_M"  || 
     $startklasse == "F_BW_M") {
    $html_startbuch .= '<div class="formHead">Startbuchdaten Rock\'n\'Roll Formationen</div>';     
  } elseif(substr($startklasse,0,3) == "BW_") {
    $html_startbuch .= '<div class="formHead">Startkarten Boogie-Woogie</div>';
  } else {
    $html_startbuch .= '<div class="formHead">Startbuchdaten Rock\'n\'Roll</div>';
  } 
  ...
  if(substr($startklasse,0,3) == "BW_"){
    $html_startbuch .= '<tr bgcolor=e9e9e9><td>Startklasse</td><td colspan=2>' . $startklasse . " ";
    ...
    $html_startbuch .= '<tr bgcolor=e9e9e9><td width=170>Startbuchnummer</td><td colspan=4>' . $startbuch_nr . '</td></tr>';
    $html_startbuch .= '<tr bgcolor=e9e9e9><td>Verein</td><td colspan=4>' . utf8_encode($vereinsname) . '</td></tr>';
    $html_startbuch .= '<tr bgcolor=e9e9e9><td>Startklasse</td><td colspan=4>' . $startklasse . '</td></tr>';  
  }
  
  if(substr($startklasse,0,3) == "BW_"){
    $html_startbuch .= '<tr><td>Name Herr</td><td colspan=2>' . utf8_encode($vn_herr) . " " . utf8_encode($nn_herr) . '</td></tr>';
    $html_startbuch .= '<tr><td>Verein Herr</td><td colspan=2>' . utf8_encode($vereinsname) . '</td></tr>';
    $html_startbuch .= '<tr><td width=170>Startkarten Herr</td><td colspan=2>' . $startbuch_nr . $startk_bw_hr .'</td></tr>';
    $html_startbuch .= '<tr><td>Startkarte Herr g&uuml;ltig</td>';
    if($gueltig == 1) {
      $html_startbuch .= '<td colspan=2 bgcolor=33cc00>JA</td></tr>';
    } else {   
      $html_startbuch .= '<td colspan=2 bgcolor=ff0033>Nein</td></tr>';
    }
    $html_startbuch .= '<tr><td>Name Dame</td><td colspan=2>' . utf8_encode($vn_dame) . " " . utf8_encode($nn_dame) . '</td></tr>';
    $html_startbuch .= '<tr><td>Verein Dame</td><td colspan=2>' . utf8_encode($verein_dame) . '</td></tr>';
    $html_startbuch .= '<tr><td width=170>Startkarte Dame</td><td colspan=2>' . utf8_encode($startbuch_nr_d) . $startk_bw_da .'</td></tr>';
    $html_startbuch .= '<tr><td>Startkarte Dame g&uuml;ltig</td>';
    if($gueltig_dame == 1) {
      $html_startbuch .= '<td colspan=2 bgcolor=33cc00>JA</td></tr>';
    } else {   
      $html_startbuch .= '<td colspan=2 bgcolor=ff0033>Nein</td></tr>'; 
    ...
    $html_startbuch .= '<tr><td bgcolor=e9e9e9>Startbuch g&uuml;ltig</td>';
    if ($gueltig == 1) {
      $html_startbuch .= '<td colspan=4 bgcolor=33cc00>JA</td></tr>';
    } else {   
      $html_startbuch .= '<td colspan=4 bgcolor=ff0033>Nein</td></tr>';
    }  
  }      
  ...
  if($_POST["passwort"] != '' && $_POST["passwort"] == $wertungsfreigabe){
    ...
    $html_startbuch .=  '<h4>Das Passwort für die Wertungsfreigabe ist falsch bzw. wurde noch nicht gesetzt!</h4>';
...
if($startbuch_nr != $_POST["mitglied_nummer"]){
  echo '<h4>Dieses Startbuch ist nicht vorhanden!<br>Bitte erneut versuchen!</h4>';
}
