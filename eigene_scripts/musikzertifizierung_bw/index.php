...
if($_GET['file'])
{
$dir = '../../adm_my_files/download/Turniermusik/Boogie-Woogie/';
...
echo'<h2>Musikzertifizierung BW Prüfung</h2>';
...
echo'<table border="1">';
echo'<tr><th>Auswahl</th><th>Titel</th><th colspan = "2">Thomas ';

if($_POST['bewerter'] == 'Thomas')
    echo'<input type="radio" name = "bewerter" value = "Thomas" checked>';
else
    echo'<input type="radio" name = "bewerter" value = "Thomas" onclick="submit();">';

echo'</th><th colspan = "2">Elian ';
if($_POST['bewerter'] == "Elian")
    echo'<input type="radio" name = "bewerter" value = "Elian" checked>';
else
    echo'<input type="radio" name = "bewerter" value = "Elian" onclick="submit();">';

echo'</th><th colspan = "2">Michael ';

if($_POST['bewerter'] == "Michael")
    echo'<input type="radio" name = "bewerter" value = "Michael" checked>';
else
    echo'<input type="radio" name = "bewerter" value = "Michael" onclick="submit();">';
    
echo'</th></tr>';

$sqlab = "SELECT * FROM zertifizierungen_bw ORDER BY last_update DESC";
   ...
   if($temp['erledigt'] == $_SESSION['anzeige'])
    {
     ...
     if(is_file('../../adm_my_files/download/Turniermusik/Boogie-Woogie/' . $temp['titel'] . '.mp3'))
        echo'<td width="270"><a href="index.php?file=' . $temp['titel'] .'.mp3">' . $temp['titel'] . '.mp3</td>';
else 
...
if($_POST['absenden'])
       {
         ...
         $sqlab = "update zertifizierungen_bw set ";
         
         if($_POST['bewerter'] == 'Thomas' && $_POST['bewertung'] !="---")
             $musik =  "g1_musik = '" . $_POST['bewertung'] ."'";
         if($_POST['bewerter'] == 'Thomas' && $_POST['notiz'] !="")
         ...
         $sqlab = "SELECT * FROM zertifizierungen_bw WHERE id = '" . $_POST['id_auswahl'] . "'";
         ...
         if($ergebnis['g1_musik'] == 'io')
           {
            ...
            $sqlab = "update zertifizierungen_bw set gutachter1_ok = '1' WHERE id = '" . $_POST['id_auswahl'] . "'";
            mysqli_query($db, $sqlab);
           }
         if($ergebnis['g1_musik'] != 'io' && $ergebnis['g1_musik'] != '---') 
           {
            $zaehler_nio = $zaehler_nio + 1;
            $sqlab = "update zertifizierungen_bw set gutachter1_ok = '2' WHERE id = '" . $_POST['id_auswahl'] . "'";
            mysqli_query($db, $sqlab);
           } 
         ...
         if($ergebnis['g2_musik'] == 'io')
           {
            $zaehler_io = $zaehler_io + 1;
            $sqlab = "update zertifizierungen_bw set gutachter2_ok = '1' WHERE id = '" . $_POST['id_auswahl'] . "'";
            mysqli_query($db, $sqlab);
           } 
         if($ergebnis['g2_musik'] != 'io' && $ergebnis['g2_musik'] != '---') 
           {
            $zaehler_nio = $zaehler_nio + 1;
            $sqlab = "update zertifizierungen_bw set gutachter2_ok = '2' WHERE id = '" . $_POST['id_auswahl'] . "'";
            mysqli_query($db, $sqlab);
           } 
         ...
         if($ergebnis['g3_musik'] == 'io')
           {
            $zaehler_io = $zaehler_io + 1;
            $sqlab = "update zertifizierungen_bw set gutachter3_ok = '1' WHERE id = '" . $_POST['id_auswahl'] . "'";
            mysqli_query($db, $sqlab);
           } 
         if($ergebnis['g3_musik'] != 'io' && $ergebnis['g3_musik'] != '---') 
           {
            $zaehler_nio = $zaehler_nio + 1;
            $sqlab = "update zertifizierungen_bw set gutachter3_ok = '2' WHERE id = '" . $_POST['id_auswahl'] . "'";
            mysqli_query($db, $sqlab);
           } 
