...
echo'<h2>Musikzertifizierung Boogie Woogie</h2>';
...
echo'<table border="1">';
echo'<tr><th>Auswahl</th><th>Titel</th><th>Interpret</th><th>Takte</th><th>Ticket</th><th>übertragen</th><th>Thomas</th><th>Philipp</th><th>Hermann</th><th>erledigt</th><th>Titelname Zertifizierung</th><th>Letzte Änderung</th></tr>';
...
// print_r($ds);echo'<br>';
      ...
      if($ds['category'] == 4 && $ds['custom16'] == 'Boogie Woogie')
      {
       $tempo = $ds['custom15'];
...
// Status holen
       $sqlab = "SELECT * FROM zertifizierungen_bw WHERE titel = '" . $dateiname[$i][1] . "'";
...
// Status holen
    $sqlab = "SELECT * FROM zertifizierungen_bw WHERE titel = '" . $datei_name . "'";
    $status = mysqli_query($db, $sqlab);
    $status_ds = mysqli_fetch_array($status); 
    // print_r($status_ds);echo" oder nicht in DB<br>";
...
// Status holen
   ...
   {
   if($ds['status'] == 3 && $ds['category'] == 4 && $ds['custom16'] == 'Boogie Woogie')
    {
...
if($_POST['zertifizieren'])
  {
   ...
   $inhalt_kopf .= "Bitte gehe auf http://drbv.de/adm/eigene_scripts/musikzertifizierung_bw/index.php\nund bearbeite folgende Titel:\n\n";
   ...
   for($x = 1; $x <= count($dateiname); $x++ )
       {
         ...
         $ziel = '../../adm_my_files/download/Turniermusik/Boogie-Woogie/' . $dateiname[key($dateiname)][1] . '.mp3';
...
// alten Eintrag löschen
         if($_POST[loeschen] == 1)
            {
             $sqlab = "delete from zertifizierungen_bw WHERE titel = '" . $dateiname[key($dateiname)][1] . "'";
//           echo$sqlab. '<br>';;
             mysqli_query($db, $sqlab);
            }

         // In Datenbank schreiben
         $sqlab = "insert zertifizierungen_bw set titel = '" . $dateiname[key($dateiname)][1] . "', eingereicht='1', gutachter1='Thomas', gutachter2='Elian', gutachter3='Michael' ";
//          echo$sqlab. '<br>';;
          mysqli_query($db, $sqlab);
...
// Benachrichtigung per E-Mail
  ...
  if($inhalt_mail)
    {
     music_zert_mail_zert_bw($inhalt_kopf, $inhalt_mail);
    }
...
if($_POST['erledigt'])
  {
   $sqlab = "update zertifizierungen_bw set erledigt = '1' WHERE titel = '" . $dateiname[key($dateiname)][1] . "'"; 
//     echo$sqlab. '<br>';;
   mysqli_query($db, $sqlab);

   $quelle = '../../adm_my_files/download/Turniermusik/Boogie-Woogie/' . $dateiname[key($dateiname)][1];
   unlink($quelle);
  }
...
if($_POST['kopieren'])
  {
   ...
   for($x = 1; $x <= count($dateiname); $x++ )
        {
         $quelle = '../../adm_my_files/download/Turniermusik/Boogie-Woogie/' . $dateiname[key($dateiname)][1] . '.mp3';
