...
// Um jederzeit die Startliste schreiben zu können, die nachfolgende Zeile auskommentieren!
      ...
      { 
       echo"<h3><font color = ff0000>Die Datei darf nicht neu erstellt werden, weil möglicherweise Tanzpaare ihre Akrobatiken in der Zwischenzeit geändert haben!<br>Es wurde für die Admins eine Kopie gespeichert.<font color = 000000></h3>";  
       $datum = $_POST["datum"];
      }
   }

echo 'Turnier-Datum: ' . $datum ."<br>";
...
// echo"Inhalt: " . $inhalt ."<p>";

if(($jetzt > $null_vier && $jetzt < $null_sechs) || $man_datum > $datum_unix)
  {
// Dateinamen festlegen
     $filename = "T" . $t_num . "_Anmeldung.txt";
     ...
     fclose($fp);
     if(is_file($fn))
     echo"Die Datei $fn wurde gespeichert!<br><br>"; 
   
// Dateinamen festlegen
     $filename = "T" . $t_num . "_Anmeldung.txt";
     // Datei auf Server speichern
     $fn = "../../cms/images/Download/TurnierProgramm/startlisten/" . $filename;
      if (is_file($fn)) 
         unlink($fn);
     $fp = fopen($fn,"w"); 
     fwrite($fp, $inhalt);
  ...
  {
// Dateinamen festlegen
     $filename = "Kopie_T" . $t_num . "_Anmeldung.txt";
     // Datei auf Server speichern
     $fn = "./kopien_startlisten/" . $filename;
      if (is_file($fn)) 
         unlink($fn);
