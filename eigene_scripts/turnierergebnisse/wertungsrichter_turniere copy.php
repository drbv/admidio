...
echo'<center><table>';
echo'<tr><td><b>Bitte Wertungsrichter auswählen: ';
echo'<select name="lizenznr" size="1"  onchange="submit();">';

// Wertungsrichter aus Datenbank lesen

$sqlab = "SELECT turniernummer,lizenznummer,name FROM wertungsrichter ORDER BY lizenznummer";
$wertungsrichter = mysqli_query($db, $sqlab);

while($temp = mysqli_fetch_array($wertungsrichter))
  {
   if(!in_array($temp[1], $wr_vorhanden))
      {
       $wr_name = utf8_encode($temp[2]);  
       if($_SESSION["lizenznr"] == $temp[1])
          echo'<option value="' . $temp[1] . '"  selected>' . $temp[1] . ' ' . $wr_name . '</option>';
       else
          echo'<option value="' . $temp[1] . '">' . $temp[1] . ' ' . $wr_name . '</option>';
